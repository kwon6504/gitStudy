...
//		xx.clear();		//싹다 지움. 수량도 0됨.
		...
		for(int i=0;i<xx.size();i=i+1) {
			System.out.println(xx.get(i)); //get은 가져오는 함수 index는 0부터 시작함으로 0 1 2가 가져와진다.
		}
		boolean hasCat = xx.contains("고양이"); //xx라는 배열안에 고양이가 있냐고 물어보는 함수
		if(hasCat == true) { // boolean은 true 또는 false만을 가지는 함수이다 조건문이나 논리연산에서 사용한다. 
