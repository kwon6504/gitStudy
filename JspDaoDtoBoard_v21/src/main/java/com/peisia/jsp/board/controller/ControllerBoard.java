...
public class ControllerBoard extends HttpServlet {
	...
	@Override
	public void init() throws ServletException { //최초의 접속때 초기화해주는 함수
		service = new ServiceBoard();
	}
	@Override
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		category = request.getParameter("category"); //category에 내가 선택한 게시판이 나온다.
		...
		if(action!=null) {
			...
			case "/list":
				...
				request.setAttribute("blp", blp); //키 값 저장 후 forward로 불러와야한다.
				break;
			}
			RequestDispatcher d = request.getRequestDispatcher(nextPage); //완료후 리턴하기위한 함수
			d.forward(request,response); //나와있는 jsp경로로 전달하는 함수.
		}
	}
}