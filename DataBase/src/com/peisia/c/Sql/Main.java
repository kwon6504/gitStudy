...
public class Main {
	...
	static private void initDb() {
		try {
			Class.forName("com.mysql.cj.jdbc.Driver"); //cj가 필수
	...
	static private void dbRun() {
		try { // 아래 코드들을 이 try{ 블럭 안에 안쓰면 에러남.
				// *db명* *id* *암호*
			Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/my_cat", "root", "root"); //getConnection 통로를 만든다.
			...
			ResultSet result = st.executeQuery("select * from board where b_no=1"); // 전송 할 sql 문
			while (result.next()) { // 결과를 하나씩 빼기. 더 이상 없으면(행 수가 끝나면) false 리턴됨.
				String text = result.getString("b_text"); // p_name 필드(열) 의 데이터 꺼내기(1개 꺼낸거에서)
