...
//	public void getList(@RequestParam(value="currentPage", defaultValue = "1") int currentPage, Model model) {
	public void getList(@RequestParam("currentPage") int currentPage, Model model) {
		...
		model.addAttribute("paging",service.pageBlock(currentPage,null));
	}	// 위 /getList 와 동일한 jsp파일을 염. 상위 경로 포함(/guest). 즉 PJ루트/guest/getList.jsp 파일을 염.
	// 그리고 이 파일은 
	// PJ\src\main\webapp\WEB-INF\views\guest\getList.jsp
	// 에 만들어 놓으면 됨.
	@GetMapping("/search")
	public void getsearch(@RequestParam("currentPage") int currentPage,@RequestParam("word") String word, Model model) {
		log.info("여기 들어오긴하니?");
		log.info(word);
		model.addAttribute("search",service.searchList(currentPage,word));
		model.addAttribute("searchpaging",service.pageBlock(currentPage,word));
...
//	}
	...
	public String del(@RequestParam("bno") Long bno) {
		log.info("컨트롤러 ==== 글번호 ==============="+bno);
		service.del(bno);
		return "redirect:/guest/getList?currentPage=1";	//sendRedirect 로 이동하게 됨.
	}
	...
	public String write(GuestDto dto) {
		service.write(dto);
		return "redirect:/guest/getList?currentPage=1";	//sendRedirect 로 이동하게 됨. // 책 p.245 참고
	}	
