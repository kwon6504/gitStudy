...
public class Dao { //Data Access Object의 약자로 
	Connection con = null;
	Statement st = null;
	/* (1/5)삭제 */
	public void del(String no) {
		try {
			...
			////고정 4
			st.close(); //서버 안정화 하기 위한 함수 ex)서버에 인원은 제한이 있지만 한번들어가면 접속중으로 되있기 때문에 서버에서 나가는 공용함수
			////고정 5
			con.close();
		} catch (Exception e) {
			e.printStackTrace();
...
//			insert into ps_board_free (b_title,b_id,b_text) values ('야옹','cat','aaa');
	...
	/* (3/5)글 읽기 */
	public Dto read(String no) { //리턴함으로서 Dto에 객체 한개를 가져오는거고
		Dto post = null;
		try {
			Class.forName(Db.DB_JDBC_DRIVER_PACKAGE_PATH);	// [고정-1]
			con = DriverManager.getConnection(Db.DB_URL, Db.DB_ID, Db.DB_PW);	// [고정-2]			
...
//			select * from ps_board_free where b_no=4;
	...
	/* (4/5)글 리스트 */
	public ArrayList<Dto> list() { //리턴으로 ArrayList를 반환한다.
